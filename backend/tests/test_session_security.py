...
from services.session_service import describe_device, hash_token, new_token, valid_pin, valid_token
...
def test_pin_is_exactly_six_ascii_digits() -> None:
    assert valid_pin("012345")
    assert not valid_pin("12345")
    assert not valid_pin("1234567")
    assert not valid_pin("12345a")
