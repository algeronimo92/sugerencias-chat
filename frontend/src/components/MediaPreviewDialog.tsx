...
function fileExtensionLabel(filename: string, contentType?: string): string {
  ...
  const fromType = /\/([a-z0-9.+-]+)$/i.exec(contentType ?? '')
  if (fromType) return fromType[1].replace(/^x-/, '').toUpperCase()
  return 'ARCHIVO'
}
...
export function MediaPreviewDialog({
  ...
  filename?: string
  /** MIME del archivo; respaldo para el subtítulo de la tarjeta de documento
   * cuando el nombre no trae extensión reconocible. */
  contentType?: string
              ...
              <div className="flex w-full max-w-sm flex-col items-center gap-5 rounded-3xl border border-white/10 bg-[#111b21] px-8 py-10 text-center shadow-2xl">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-sm text-[#8696a0]">
                    {fileExtensionLabel(filename || '', contentType)}
