...
def audio_route(monkeypatch):
    ...
    monkeypatch.setattr(chats, "_resolve_reply_to", AsyncMock(return_value=None))
    monkeypatch.setattr(chats, "enqueue_messages", AsyncMock(return_value=[{"id": 1, "status": "PENDING"}]))
    monkeypatch.setattr(chats.manager, "broadcast", AsyncMock())
    monkeypatch.setattr(chats.base64, "b64decode", counting_decode)
...
async def test_transcoded_audio_is_decoded_once_and_never_re_encoded(audio_route, monkeypatch):
    ...
    monkeypatch.setattr(chats, "transcode_audio_to_ogg_opus", lambda raw: b"OGG-" + raw)
    monkeypatch.setattr(chats, "save_decoded_media", lambda content_type, raw: saved.append((content_type, raw)) or "/media/a.ogg")
...
async def test_audio_that_cannot_be_transcoded_keeps_the_original_bytes(audio_route, monkeypatch):
    saved: list[tuple[str, bytes]] = []
    monkeypatch.setattr(chats, "transcode_audio_to_ogg_opus", lambda raw: (_ for _ in ()).throw(chats.AudioTranscodeError("sin ffmpeg")))
    monkeypatch.setattr(chats, "save_decoded_media", lambda content_type, raw: saved.append((content_type, raw)) or "/media/a.webm")
...
async def test_invalid_base64_is_rejected_before_touching_storage(audio_route, monkeypatch):
    monkeypatch.setattr(chats, "save_decoded_media", lambda *_args: pytest.fail("no debe guardar nada"))
