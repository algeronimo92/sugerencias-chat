...
import {
  Activity, AlertTriangle, Bot, CheckCircle2, ChevronDown, Clock3,
  GitBranch, History, Loader2, Pencil, Plus, Power, Save, Trash2, XCircle,
} from 'lucide-react'
import type {
  AutomationAction, AutomationActionType, AutomationConditions, AutomationRule,
  AutomationTrigger, LeadStage, TaskPriority, TaskType,
} from '../types'
import { LEAD_STAGES } from '../types'
import {
  useAutomationExecutions, useAutomationRules, useCreateAutomation, useUpdateAutomation,
...
const TRIGGERS: Array<{ value: AutomationTrigger; label: string; description: string }> = [
  { value: 'lead_created', label: 'Lead nuevo', description: 'Cuando se registra un contacto por primera vez.' },
  { value: 'stage_changed', label: 'Cambio de etapa', description: 'Cuando una persona o el agente mueve el lead.' },
  { value: 'message_received', label: 'Mensaje recibido', description: 'Cuando llega un nuevo mensaje del cliente.' },
  { value: 'seller_response_overdue', label: 'Vendedor sin responder', description: 'El último mensaje es del cliente y vence el tiempo configurado.' },
  { value: 'customer_response_overdue', label: 'Cliente sin responder', description: 'El último mensaje es del vendedor y vence el tiempo configurado.' },
  { value: 'task_due', label: 'Tarea vencida', description: 'Cuando una tarea pendiente llega a su fecha límite.' },
]
...
const EMPTY_FORM: RuleForm = {
  name: '', triggerType: 'lead_created', triggerMinutes: 30, delayMinutes: 0,
  conditions: { stage: null, origin_contains: '', service_contains: '', seller_id: null, tag_id: null, require_open_window: false, business_hours_only: false },
  actions: [defaultAction('create_task')], isActive: true,
}
...
function executionTone(status: string) {
  if (status === 'completed') return 'bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-300'
  if (status === 'failed') return 'bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300'
  if (status === 'skipped') return 'bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300'
...
function validateForm(form: RuleForm) {
  ...
  if (['seller_response_overdue', 'customer_response_overdue'].includes(form.triggerType) && (!form.triggerMinutes || form.triggerMinutes < 1 || form.triggerMinutes > 43200)) errors.push('La demora debe estar entre 1 minuto y 30 días.')
  ...
  form.actions.forEach((action, index) => {
    ...
    if (action.type === 'create_task' && (!action.title?.trim() || !action.due_minutes || action.due_minutes < 1)) errors.push(`${prefix}: completa el título y vencimiento de la tarea.`)
    if (action.type === 'create_task' && (action.remind_minutes_before ?? 0) >= (action.due_minutes ?? 0)) errors.push(`${prefix}: el recordatorio debe ser anterior al vencimiento.`)
    if (action.type === 'assign_seller' && !action.user_id) errors.push(`${prefix}: selecciona un vendedor.`)
    if ((action.type === 'add_tag' || action.type === 'remove_tag') && !action.tag_id) errors.push(`${prefix}: selecciona una etiqueta.`)
    if (action.type === 'notify' && (!action.title?.trim() || !action.body?.trim() || (action.recipient === 'specific' && !action.user_id))) errors.push(`${prefix}: completa destinatario, título y contenido.`)
    if (action.type === 'send_template' && !action.template_id) errors.push(`${prefix}: selecciona una plantilla.`)
  })
  return errors
}
...
export function AutomationsPage() {
  ...
  const simpleRules = rules.filter(rule => rule.builder_mode !== 'visual')
  const visualRules = rules.filter(rule => rule.builder_mode === 'visual')
  ...
  function setAction(index: number, patch: Partial<AutomationAction>) {
    setForm(current => ({ ...current, actions: current.actions.map((action, actionIndex) => actionIndex === index ? { ...action, ...patch } as AutomationAction : action) }))
  }
  ...
  function submit(event: React.FormEvent) {
    ...
    const input = {
      ...
      trigger_config: ['seller_response_overdue', 'customer_response_overdue'].includes(form.triggerType) ? { minutes: Number(form.triggerMinutes) } : {},
      conditions: {
    ...
    <div className="h-full overflow-y-auto bg-gray-50 p-4 dark:bg-gray-950 sm:p-6">
      ...
      {formOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-3" onMouseDown={event => { if (event.target === event.currentTarget) closeForm() }}><form onSubmit={submit} className="automation-form flex max-h-[94vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-900">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
          <section className="grid gap-3 md:grid-cols-2"><label className="grid gap-1 text-xs font-semibold text-gray-600 dark:text-gray-300">Nombre<input required maxLength={120} value={form.name} onChange={event => setForm(current => ({ ...current, name: event.target.value }))} placeholder="Ej. Seguimiento de leads nuevos" className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800" /></label><label className="grid gap-1 text-xs font-semibold text-gray-600 dark:text-gray-300">Disparador<select value={form.triggerType} onChange={event => setForm(current => ({ ...current, triggerType: event.target.value as AutomationTrigger }))} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800">{TRIGGERS.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><p className="text-[11px] text-gray-500 md:col-span-2">{TRIGGERS.find(item => item.value === form.triggerType)?.description}</p>{['seller_response_overdue', 'customer_response_overdue'].includes(form.triggerType) && <label className="grid gap-1 text-xs font-semibold text-gray-600 dark:text-gray-300">Minutos de espera<input required type="number" min={1} max={43200} value={form.triggerMinutes} onChange={event => setForm(current => ({ ...current, triggerMinutes: Number(event.target.value) }))} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800" /></label>}<label className="grid gap-1 text-xs font-semibold text-gray-600 dark:text-gray-300">Retrasar toda la regla (minutos)<input type="number" min={0} max={10080} value={form.delayMinutes} onChange={event => setForm(current => ({ ...current, delayMinutes: Number(event.target.value) }))} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800" /></label></section>

          <section><h3 className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-gray-700 dark:text-gray-200"><GitBranch className="h-3.5 w-3.5" />Condiciones opcionales</h3><div className="grid gap-3 rounded-xl border border-gray-200 p-3 dark:border-gray-700 md:grid-cols-3"><label className="grid gap-1 text-[11px] text-gray-500">Etapa<select value={form.conditions.stage ?? ''} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, stage: (event.target.value || null) as LeadStage | null } }))} className="rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800"><option value="">Cualquiera</option>{LEAD_STAGES.map(stage => <option key={stage} value={stage}>{stage}</option>)}</select></label><label className="grid gap-1 text-[11px] text-gray-500">Origen contiene<input maxLength={120} value={form.conditions.origin_contains ?? ''} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, origin_contains: event.target.value } }))} placeholder="Facebook" className="rounded-lg border border-gray-200 px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800" /></label><label className="grid gap-1 text-[11px] text-gray-500">Servicio contiene<input maxLength={120} value={form.conditions.service_contains ?? ''} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, service_contains: event.target.value } }))} placeholder="Limpieza" className="rounded-lg border border-gray-200 px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800" /></label><label className="grid gap-1 text-[11px] text-gray-500">Vendedor<select value={form.conditions.seller_id ?? ''} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, seller_id: event.target.value ? Number(event.target.value) : null } }))} className="rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800"><option value="">Cualquiera</option>{activeUsers.map(user => <option key={user.id} value={user.id}>{user.name}</option>)}</select></label><label className="grid gap-1 text-[11px] text-gray-500">Etiqueta<select value={form.conditions.tag_id ?? ''} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, tag_id: event.target.value ? Number(event.target.value) : null } }))} className="rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800"><option value="">Cualquiera</option>{tags.map(tag => <option key={tag.id} value={tag.id}>{tag.name}</option>)}</select></label><div className="flex flex-col justify-end gap-2 pb-1"><label className="flex items-center gap-2 text-[11px] text-gray-600 dark:text-gray-300"><input type="checkbox" checked={!!form.conditions.require_open_window} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, require_open_window: event.target.checked } }))} />Ventana de WhatsApp abierta</label><label className="flex items-center gap-2 text-[11px] text-gray-600 dark:text-gray-300"><input type="checkbox" checked={!!form.conditions.business_hours_only} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, business_hours_only: event.target.checked } }))} />Horario laboral 08:00–18:00</label></div></div></section>

          <section><div className="mb-2 flex items-center justify-between"><h3 className="flex items-center gap-1.5 text-xs font-semibold text-gray-700 dark:text-gray-200"><Activity className="h-3.5 w-3.5" />Acciones en orden</h3><button type="button" disabled={form.actions.length >= 10} onClick={() => setForm(current => ({ ...current, actions: [...current.actions, defaultAction('create_task')] }))} className="flex items-center gap-1 text-xs font-semibold text-green-700 disabled:opacity-40 dark:text-green-400"><Plus className="h-3.5 w-3.5" />Agregar</button></div><div className="space-y-3">{form.actions.map((action, index) => <div key={index} className="rounded-xl border border-violet-200 bg-violet-50/30 p-3 dark:border-violet-900 dark:bg-violet-950/10"><div className="mb-3 flex items-center gap-2"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-violet-600 text-[10px] font-bold text-white">{index + 1}</span><select value={action.type} onChange={event => setForm(current => ({ ...current, actions: current.actions.map((item, itemIndex) => itemIndex === index ? defaultAction(event.target.value as AutomationActionType) : item) }))} className="min-w-0 flex-1 rounded-lg border border-violet-200 bg-white px-2.5 py-2 text-xs dark:border-violet-900 dark:bg-gray-800">{Object.entries(ACTION_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><button type="button" disabled={form.actions.length === 1} onClick={() => setForm(current => ({ ...current, actions: current.actions.filter((_, itemIndex) => itemIndex !== index) }))} className="rounded-lg p-2 text-red-500 disabled:opacity-30"><Trash2 className="h-4 w-4" /></button></div>
            {action.type === 'create_task' && <div className="grid gap-2 md:grid-cols-3"><label className="grid gap-1 text-[10px] text-gray-500 dark:text-gray-400 md:col-span-2">Título de la tarea<input required maxLength={160} value={action.title ?? ''} onChange={event => setAction(index, { title: event.target.value })} placeholder="Ej. Dar seguimiento a {{nombre}}" className="rounded-lg border border-gray-200 px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800" /></label><label className="grid gap-1 text-[10px] text-gray-500 dark:text-gray-400">Tipo de tarea<select value={action.task_type} onChange={event => setAction(index, { task_type: event.target.value as TaskType })} className="rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800">{TASK_TYPES.map(type => <option key={type.value} value={type.value}>{type.label}</option>)}</select></label><label className="grid gap-1 text-[10px] text-gray-500 dark:text-gray-400 md:col-span-3">Descripción<input maxLength={1000} value={action.description ?? ''} onChange={event => setAction(index, { description: event.target.value })} placeholder="Opcional" className="rounded-lg border border-gray-200 px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800" /></label><label className="grid gap-1 text-[10px] text-gray-500 dark:text-gray-400">Vence en minutos<input type="number" min={1} max={43200} value={action.due_minutes} onChange={event => setAction(index, { due_minutes: Number(event.target.value) })} className="rounded-lg border border-gray-200 px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800" /></label><label className="grid gap-1 text-[10px] text-gray-500 dark:text-gray-400">Recordar antes (min)<input type="number" min={0} value={action.remind_minutes_before} onChange={event => setAction(index, { remind_minutes_before: Number(event.target.value) })} className="rounded-lg border border-gray-200 px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800" /></label><label className="grid gap-1 text-[10px] text-gray-500 dark:text-gray-400">Responsable<select value={action.assigned_user_id ?? ''} onChange={event => setAction(index, { assigned_user_id: event.target.value ? Number(event.target.value) : null })} className="rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800"><option value="">Vendedor del lead</option>{activeUsers.map(user => <option key={user.id} value={user.id}>{user.name}</option>)}</select></label><label className="grid gap-1 text-[10px] text-gray-500 dark:text-gray-400">Prioridad<select value={action.priority} onChange={event => setAction(index, { priority: event.target.value as TaskPriority })} className="rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800"><option value="low">Baja</option><option value="normal">Normal</option><option value="high">Alta</option></select></label></div>}
            {action.type === 'assign_seller' && <select required value={action.user_id ?? ''} onChange={event => setAction(index, { user_id: Number(event.target.value) || null })} className="w-full rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800"><option value="">Selecciona vendedor</option>{activeUsers.map(user => <option key={user.id} value={user.id}>{user.name}</option>)}</select>}
            {(action.type === 'add_tag' || action.type === 'remove_tag') && <select required value={action.tag_id ?? ''} onChange={event => setAction(index, { tag_id: Number(event.target.value) || undefined })} className="w-full rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800"><option value="">Selecciona etiqueta</option>{tags.map(tag => <option key={tag.id} value={tag.id}>{tag.name}</option>)}</select>}
            {action.type === 'change_stage' && <select value={action.stage} onChange={event => setAction(index, { stage: event.target.value as LeadStage })} className="w-full rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800">{LEAD_STAGES.map(stage => <option key={stage} value={stage}>{stage}</option>)}</select>}
            {action.type === 'notify' && <div className="grid gap-2 md:grid-cols-2"><select value={action.recipient} onChange={event => setAction(index, { recipient: event.target.value as 'seller' | 'specific', user_id: null })} className="rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800"><option value="seller">Vendedor del lead</option><option value="specific">Usuario específico</option></select>{action.recipient === 'specific' && <select required value={action.user_id ?? ''} onChange={event => setAction(index, { user_id: Number(event.target.value) || null })} className="rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800"><option value="">Selecciona usuario</option>{activeUsers.map(user => <option key={user.id} value={user.id}>{user.name}</option>)}</select>}<input required maxLength={160} value={action.title ?? ''} onChange={event => setAction(index, { title: event.target.value })} placeholder="Título" className="rounded-lg border border-gray-200 px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800 md:col-span-2" /><textarea required maxLength={1000} rows={2} value={action.body ?? ''} onChange={event => setAction(index, { body: event.target.value })} placeholder="Contenido de la notificación" className="rounded-lg border border-gray-200 px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800 md:col-span-2" /></div>}
            {action.type === 'send_template' && <div><select required value={action.template_id ?? ''} onChange={event => setAction(index, { template_id: Number(event.target.value) || undefined })} className="w-full rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-xs dark:border-gray-700 dark:bg-gray-800"><option value="">Selecciona plantilla de texto</option>{automaticTemplates.map(template => <option key={template.id} value={template.id}>{template.name}</option>)}</select><p className="mt-1 text-[10px] text-amber-700 dark:text-amber-300">Solo se envía con la ventana de 24 horas abierta. No se admiten adjuntos ni mensajes interactivos automáticos.</p></div>}
          </div>)}</div></section>
