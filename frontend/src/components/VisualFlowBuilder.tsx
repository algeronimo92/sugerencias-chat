        ...
        {group.conditions.map((condition, conditionIndex) => <div key={condition.id} className="rounded-lg border border-wa-border bg-white p-2 dark:border-wa-border-dark dark:bg-wa-head-dark">
          ...
          <div className="flex items-start gap-1.5">
            <div className="min-w-0 flex-1 space-y-2">
              ...
              {condition.condition_type === ConditionType.OriginContains && <input maxLength={120} value={String(condition.value ?? '')} onChange={event => updateCondition(group.id, condition.id, { value: event.target.value })} placeholder="Ej. Facebook" className={fieldClass} />}
              {condition.condition_type === ConditionType.ServiceContains && <input maxLength={120} value={String(condition.value ?? '')} onChange={event => updateCondition(group.id, condition.id, { value: event.target.value })} placeholder="Ej. Limpieza" className={fieldClass} />}
              {MESSAGE_CONDITION_TYPES.has(condition.condition_type) && <input maxLength={500} value={String(condition.value ?? '')} onChange={event => updateCondition(group.id, condition.id, { value: event.target.value })} placeholder="Ej. Hollywood Peel" className={fieldClass} />}
