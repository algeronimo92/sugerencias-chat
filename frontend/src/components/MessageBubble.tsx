...
export function MessageStatusTicks({
  ...
  if (status === "PENDING") {
    return (
      <span
        className="inline-flex items-center gap-1 text-wa-faint dark:text-wa-text-dark/60"
        aria-label="Enviando"
        title="Enviando"
      >
        <Loader2 aria-hidden="true" className="h-3 w-3 animate-spin" /> Enviando
      </span>
    );
  }
  if (status === "FAILED") {
    ...
    return (
      <span className="inline-flex items-center gap-1.5">
        <button
          ...
          title={errorDetail || "Reintentar envío"}
        >
          <RefreshCw aria-hidden="true" className="h-3 w-3" /> No enviado ·
          Reintentar
        </button>
        <span
          aria-hidden="true"
          className="text-wa-faint dark:text-wa-text-dark/60"
        >
          ·
        </span>
        <button
          type="button"
          onClick={onDiscard}
          className="text-wa-faint hover:text-wa-muted dark:text-wa-text-dark/60 dark:hover:text-wa-text-dark"
          aria-label="Descartar el envío fallido"
          title="Descartar: no reintentar este mensaje"
        >
          Descartar
        </button>
      </span>
  ...
  if (status === "REJECTED") {
    return (
      <span
        ...
        aria-label="Meta no pudo entregar este mensaje"
        title="Meta no pudo entregar este mensaje (ventana de 24h o plantilla no aprobada)"
      >
        <AlertCircle aria-hidden="true" className="h-3 w-3" /> No entregado
      </span>
    );
  }
  ...
  if (status === "DISCARDED") {
    return (
      <span
        className="inline-flex items-center gap-1 text-wa-faint dark:text-wa-text-dark/60"
        aria-label="No enviado, descartado"
        title="No se envió y se descartó: no se va a reintentar"
      >
        <Ban aria-hidden="true" className="h-3 w-3" /> No enviado · Descartado
      </span>
    );
  }
  if (status === "PLAYED") {
    ...
    return (
      <span aria-label={label} title={label}>
        <CheckCheck
          aria-hidden="true"
          className="w-3.5 h-3.5 text-wa-accent shrink-0"
        />
      </span>
    );
  }
  if (status === "READ") {
    return (
      <span aria-label="Leído" title="Leído">
        <CheckCheck
          aria-hidden="true"
          className="w-3.5 h-3.5 text-wa-accent shrink-0"
        />
  ...
  if (status === "DELIVERY_ACK") {
    return (
      <span aria-label="Entregado" title="Entregado">
        <CheckCheck
          aria-hidden="true"
          className="w-3.5 h-3.5 text-wa-faint dark:text-wa-text-dark/60 shrink-0"
        />
      </span>
    );
  }
  return (
    <span aria-label="Enviado" title="Enviado">
      <Check
        aria-hidden="true"
        className="w-3.5 h-3.5 text-wa-faint dark:text-wa-text-dark/60 shrink-0"
      />
...
export function MessageBubble({
  ...
  const sentAgo = m.sent_at
    ? Date.now() - new Date(m.sent_at).getTime()
    : Infinity;
  ...
  const actionButtonClass =
    "rounded-full p-1.5 text-wa-muted transition-colors hover:bg-black/5 hover:text-wa-text dark:text-wa-muted-dark dark:hover:bg-white/10 dark:hover:text-wa-text-dark";
  ...
  return (
    <>
      <div
        className={`group flex items-center gap-1 ${isVendedor ? "justify-end" : "justify-start"} ${isFirstOfGroup ? "mt-3" : "mt-0.75"} ${reactions.length > 0 ? "mb-2.5" : ""} ${
          selectionMode
        ...
        aria-label={
          selectionMode && canForward ? "Seleccionar mensaje" : undefined
        }
      >
        {selectionMode && (
          <span
        ...
        <div className="flex max-w-[85%] flex-col sm:max-w-[75%]">
          ...
          >
            ...
            {selectionMode && (
              <span
                aria-hidden="true"
                className="absolute inset-0 z-20 rounded-bubble"
              />
            )}
            {isDeleted ? (
            ...
            {(isDeleted || kind !== "video") && (
              <div className="flex items-center justify-end gap-1 text-[10px] text-wa-faint dark:text-wa-text-dark/60 mt-1">
                ...
                {!isDeleted && m.edited_at && (
                  <span title={`Editado ${formatMessageTime(m.edited_at)}`}>
                    Editado
                  </span>
                )}
                <span>{formatMessageTime(m.sent_at)}</span>
