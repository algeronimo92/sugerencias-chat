...
export function AutomationsPage() {
  ...
  const historyColumns = useMemo<ColumnDef<AutomationExecution>[]>(() => [
    { id: 'rule', accessorFn: execution => execution.rule_name },
    { id: 'lead', accessorFn: execution => execution.lead_name ?? execution.lead_id ?? '' },
    { id: 'status', accessorFn: execution => execution.status },
    { id: 'createdAt', accessorFn: execution => execution.created_at },
  ], [])
  const executionTable = useReactTable({ data: executions, columns: historyColumns, getCoreRowModel: getCoreRowModel() })
  ...
  function toggleRule(rule: AutomationRule) {
    ...
    update.mutate(
      ...
      { onSuccess: () => toast.success(rule.is_active ? 'Automatización pausada' : 'Automatización activada'), onError: reason => setError(extractErrorMessage(reason)) },
    )
  }

  function duplicateRule(rule: AutomationRule) {
    setError(null)
    duplicate.mutate(rule.id, { onSuccess: () => toast.success('Automatización duplicada'), onError: reason => setError(extractErrorMessage(reason)) })
  }

  function deleteRule(rule: AutomationRule) {
    setError(null)
    deleteAutomation.mutate(rule.id, {
      onSuccess: () => toast.success('Automatización eliminada; el historial se conservó'),
      onError: reason => setError(extractErrorMessage(reason)),
    })
  }

  function deleteControl(rule: AutomationRule) {
    const isDeleting = deleteAutomation.isPending && deleteAutomation.variables === rule.id
    return (
      <ConfirmDialog
        title={`Eliminar “${rule.name}”`}
        description={`La automatización dejará de ejecutarse y desaparecerá de esta lista. ${rule.execution_count ? `Sus ${rule.execution_count} ejecuciones y versiones se conservarán en el historial.` : 'No tiene ejecuciones registradas.'}`}
        confirmLabel="Eliminar automatización"
      ...
      >
        <button
          ...
          aria-label={isDeleting ? 'Eliminando automatización' : `Eliminar ${rule.name}`}
          title={isDeleting ? 'Eliminando…' : 'Eliminar'}
          className="rounded-lg p-2 text-red-500 hover:bg-red-50 disabled:cursor-wait disabled:opacity-40 dark:text-red-400 dark:hover:bg-red-950/40"
        >
    ...
    <div className="h-full overflow-y-auto bg-wa-app p-4 dark:bg-wa-app-dark sm:p-6">
      <div className="mx-auto max-w-6xl">
        ...
        {tab === 'rules' ? (
          ...
          isLoading ? <div className="flex justify-center py-20"><Loader2 className="h-6 w-6 animate-spin text-wa-muted" /></div> : visualRules.length === 0 ? <div className="rounded-2xl border border-dashed border-gray-300 bg-white px-6 py-16 text-center dark:border-wa-border-dark dark:bg-wa-panel-dark"><Bot className="mx-auto h-10 w-10 text-gray-300" /><p className="mt-3 text-sm font-semibold text-gray-700 dark:text-wa-text-dark">Todavía no hay flujos visuales</p><p className="mt-1 text-xs text-wa-muted">Conecta disparadores, condiciones, acciones y esperas en un lienzo.</p><button type="button" onClick={() => setFlowBuilder({})} className="mt-4 rounded-lg bg-wa-primary px-3 py-2 text-xs font-semibold text-white"><Plus className="mr-1 inline h-3.5 w-3.5" />Crear primer flujo</button></div> : <div className="grid gap-3 lg:grid-cols-2">{visualRules.map(rule => {
            ...
            return <article key={rule.id} className={`rounded-2xl border bg-white p-4 shadow-sm dark:bg-wa-panel-dark ${rule.is_active ? 'border-green-200 dark:border-green-900' : 'border-wa-border dark:border-wa-border-dark'}`}><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-violet-100 text-violet-700 dark:bg-violet-950 dark:text-violet-300"><GitBranch className="h-5 w-5" /></span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><h2 className="truncate text-sm font-semibold text-wa-text dark:text-white">{rule.name}</h2><p className="mt-0.5 text-[11px] text-wa-muted">{nodes} bloques · {triggerLabel(rule.trigger_type)}</p></div><div className="flex items-center gap-1"><button type="button" onClick={() => setFlowBuilder({ rule })} title="Abrir constructor" className="rounded-lg p-2 text-wa-muted hover:bg-wa-field hover:text-gray-700 dark:hover:bg-wa-head-dark"><Pencil className="h-3.5 w-3.5" /></button><button type="button" disabled={duplicate.isPending} aria-busy={duplicate.isPending && duplicate.variables === rule.id} onClick={() => duplicateRule(rule)} title={duplicate.isPending && duplicate.variables === rule.id ? 'Duplicando…' : 'Duplicar'} className="rounded-lg p-2 text-wa-muted hover:bg-wa-field hover:text-gray-700 disabled:cursor-wait disabled:opacity-40 dark:hover:bg-wa-head-dark">{duplicate.isPending && duplicate.variables === rule.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Copy className="h-3.5 w-3.5" />}</button>{hasPublished && <button type="button" disabled={update.isPending} aria-busy={update.isPending && update.variables?.id === rule.id} onClick={() => toggleRule(rule)} title={update.isPending && update.variables?.id === rule.id ? 'Actualizando…' : rule.is_active ? 'Pausar' : 'Reanudar'} className={`rounded-lg p-2 disabled:cursor-wait disabled:opacity-40 ${rule.is_active ? 'text-wa-primary-strong hover:bg-green-50 dark:hover:bg-green-950' : 'text-wa-muted hover:bg-wa-field dark:hover:bg-wa-head-dark'}`}>{update.isPending && update.variables?.id === rule.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Power className="h-3.5 w-3.5" />}</button>}{deleteControl(rule)}</div></div><div className="mt-3 flex flex-wrap gap-1.5"><span className={`rounded-full px-2 py-1 text-[10px] font-semibold ${rule.is_active ? 'bg-green-100 text-wa-primary-strong dark:bg-green-950 dark:text-green-300' : hasPublished ? 'bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300' : 'bg-wa-field text-gray-600 dark:bg-wa-head-dark dark:text-gray-300'}`}>{rule.is_active ? `Publicado · v${rule.flow_version}` : hasPublished ? 'Pausado' : 'Borrador'}</span>{hasDraftChanges && <span className="rounded-full bg-blue-100 px-2 py-1 text-[10px] font-semibold text-blue-700 dark:bg-blue-950 dark:text-blue-300">Cambios sin publicar</span>}</div><div className="mt-3 flex items-center justify-between border-t border-wa-border pt-3 text-[10px] text-wa-muted dark:border-wa-border-dark"><span>{rule.execution_count} ejecuciones · última {formatDate(rule.last_execution_at)}</span><button type="button" onClick={() => setFlowBuilder({ rule })} className="font-semibold text-wa-primary-strong">Editar flujo →</button></div></div></div></article>
          })}</div>
        ) : (
          <div className="overflow-hidden rounded-2xl border border-wa-border bg-white dark:border-wa-border-dark dark:bg-wa-panel-dark">
            <div className="flex flex-wrap items-end gap-3 border-b border-wa-border bg-wa-hover/60 p-3 dark:border-wa-border-dark dark:bg-wa-head-dark/40">
              <label className="grid min-w-48 flex-1 gap-1 text-[10px] font-semibold uppercase tracking-wide text-wa-muted">Automatización
                <Select value={historyRuleId ?? ''} onChange={event => setHistoryRuleId(event.target.value ? Number(event.target.value) : null)} className="rounded-lg border border-wa-border bg-white px-2.5 py-2 text-xs font-normal normal-case tracking-normal text-gray-700 dark:border-wa-border-dark dark:bg-wa-head-dark dark:text-wa-text-dark">
              ...
              {hasCustomHistoryFilters && <button type="button" onClick={resetHistoryFilters} className="min-h-9 rounded-lg px-3 py-2 text-xs font-semibold text-wa-primary-strong hover:bg-green-50 dark:text-wa-primary dark:hover:bg-green-950/40">Limpiar filtros</button>}
              <span className="ml-auto flex min-h-9 items-center gap-1.5 text-[11px] text-wa-muted">{executionsFetching && <Loader2 className="h-3.5 w-3.5 animate-spin" />}{executions.length} resultados</span>
            </div>
            {executionsLoading ? <div className="flex justify-center px-4 py-16"><Loader2 className="h-6 w-6 animate-spin text-wa-muted" /></div> : executions.length === 0 ? <p className="px-4 py-16 text-center text-sm text-wa-muted">No hay ejecuciones que coincidan con estos filtros.</p> : executionTable.getRowModel().rows.map(({ original: execution }) => (
              <details key={execution.id} className="group border-b border-wa-border last:border-0 dark:border-wa-border-dark">
                <summary className="flex cursor-pointer list-none items-center gap-3 px-4 py-3 hover:bg-wa-hover dark:hover:bg-wa-head-dark/60"><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${executionTone(execution.status)}`}>{execution.status === AutomationExecutionStatus.Completed ? <CheckCircle2 className="h-4 w-4" /> : execution.status === AutomationExecutionStatus.Failed ? <XCircle className="h-4 w-4" /> : execution.status === AutomationExecutionStatus.Skipped ? <AlertTriangle className="h-4 w-4" /> : <Clock3 className="h-4 w-4" />}</span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold text-gray-800 dark:text-wa-text-dark">{execution.rule_name}</span><span className="block truncate text-[10px] text-wa-muted">{execution.lead_name || execution.lead_id || 'Lead eliminado'} · {triggerLabel(execution.trigger_type)} · {formatDate(execution.created_at)}</span></span><span className={`rounded-full px-2 py-1 text-[10px] font-semibold ${executionTone(execution.status)}`}>{executionStatusLabel(execution.status)}</span><ChevronDown className="h-4 w-4 text-wa-muted transition-transform group-open:rotate-180" /></summary>
                {execution.rule_deleted && <div className="border-t border-wa-border bg-amber-50 px-4 py-2 text-[11px] text-amber-700 dark:border-wa-border-dark dark:bg-amber-950/30 dark:text-amber-300">Automatización eliminada · historial conservado solo para auditoría.</div>}
                <div className="bg-wa-hover px-4 py-3 text-xs dark:bg-wa-app-dark/60">
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    {!execution.rule_deleted && (execution.status === AutomationExecutionStatus.Scheduled || execution.status === AutomationExecutionStatus.Running || execution.status === AutomationExecutionStatus.Paused) && <button
                      ...
                      onClick={() => { setError(null); cancelExecution.mutate(execution.id, { onSuccess: () => toast.success('Ejecución cancelada'), onError: reason => setError(extractErrorMessage(reason)) }) }}
                      className="flex items-center gap-1.5 rounded-lg border border-red-500 px-3 py-1.5 text-[11px] font-semibold text-red-600 disabled:cursor-wait disabled:opacity-40 dark:text-red-400"
                    >
                      {cancelExecution.isPending && cancelExecution.variables === execution.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Ban className="h-3.5 w-3.5" />}
                      {cancelExecution.isPending && cancelExecution.variables === execution.id ? 'Cancelando…' : 'Cancelar'}
                    </button>}
                  </div>
                </div>
              </details>
      ...
      {formOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-3" onMouseDown={event => { if (event.target === event.currentTarget) closeForm() }}><form onSubmit={submit} className="automation-form flex max-h-[94vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-wa-border bg-white shadow-2xl dark:border-wa-border-dark dark:bg-wa-panel-dark">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
          <section className="grid gap-3 md:grid-cols-2"><label className="grid gap-1 text-xs font-semibold text-gray-600 dark:text-gray-300">Nombre<input required maxLength={120} value={form.name} onChange={event => setForm(current => ({ ...current, name: event.target.value }))} placeholder="Ej. Seguimiento de leads nuevos" className="rounded-lg border border-wa-border bg-white px-3 py-2 text-sm dark:border-wa-border-dark dark:bg-wa-head-dark" /></label><label className="grid gap-1 text-xs font-semibold text-gray-600 dark:text-gray-300">Disparador<Select value={form.triggerType} onChange={event => { const value = event.target.value; if (isAutomationTrigger(value)) setForm(current => ({ ...current, triggerType: value })) }} className="rounded-lg border border-wa-border bg-white px-3 py-2 text-sm dark:border-wa-border-dark dark:bg-wa-head-dark">{TRIGGERS.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}</Select></label><p className="text-[11px] text-wa-muted md:col-span-2">{TRIGGERS.find(item => item.value === form.triggerType)?.description}</p>{RESPONSE_OVERDUE_TRIGGERS.has(form.triggerType) && <label className="grid gap-1 text-xs font-semibold text-gray-600 dark:text-gray-300">Minutos de espera<input required type="number" min={1} max={43200} value={form.triggerMinutes} onChange={event => setForm(current => ({ ...current, triggerMinutes: Number(event.target.value) }))} className="rounded-lg border border-wa-border bg-white px-3 py-2 text-sm dark:border-wa-border-dark dark:bg-wa-head-dark" /></label>}<label className="grid gap-1 text-xs font-semibold text-gray-600 dark:text-gray-300">Retrasar toda la regla (minutos)<input type="number" min={0} max={10080} value={form.delayMinutes} onChange={event => setForm(current => ({ ...current, delayMinutes: Number(event.target.value) }))} className="rounded-lg border border-wa-border bg-white px-3 py-2 text-sm dark:border-wa-border-dark dark:bg-wa-head-dark" /></label><label className="grid gap-1 text-xs font-semibold text-gray-600 dark:text-gray-300">Máximo de ejecuciones por hora<input type="number" min={1} max={1000} placeholder="Sin límite" value={form.maxExecutionsPerHour ?? ''} onChange={event => setForm(current => ({ ...current, maxExecutionsPerHour: event.target.value ? Number(event.target.value) : null }))} className="rounded-lg border border-wa-border bg-white px-3 py-2 text-sm dark:border-wa-border-dark dark:bg-wa-head-dark" /><span className="text-[10px] font-normal text-wa-muted">Freno de seguridad: pasado el tope, el resto se reintenta más tarde en vez de perderse.</span></label></section>

          <section><h3 className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-gray-700 dark:text-wa-text-dark"><GitBranch className="h-3.5 w-3.5" />Condiciones opcionales</h3><div className="grid gap-3 rounded-xl border border-wa-border p-3 dark:border-wa-border-dark md:grid-cols-3"><label className="grid gap-1 text-[11px] text-wa-muted">Etapa<Select value={form.conditions.stage ?? ''} onChange={event => { const value = event.target.value; const stage = isLeadStage(value) ? value : null; setForm(current => ({ ...current, conditions: { ...current.conditions, stage } })) }} className="rounded-lg border border-wa-border bg-white px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark"><option value="">Cualquiera</option>{LEAD_STAGES.map(stage => <option key={stage} value={stage}>{stage}</option>)}</Select></label><label className="grid gap-1 text-[11px] text-wa-muted">Origen contiene<input maxLength={120} value={form.conditions.origin_contains ?? ''} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, origin_contains: event.target.value } }))} placeholder="Facebook" className="rounded-lg border border-wa-border px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark" /></label><label className="grid gap-1 text-[11px] text-wa-muted">Servicio contiene<input maxLength={120} value={form.conditions.service_contains ?? ''} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, service_contains: event.target.value } }))} placeholder="Limpieza" className="rounded-lg border border-wa-border px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark" /></label><label className="grid gap-1 text-[11px] text-wa-muted">Vendedor<Select value={form.conditions.seller_id ?? ''} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, seller_id: event.target.value ? Number(event.target.value) : null } }))} className="rounded-lg border border-wa-border bg-white px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark"><option value="">Cualquiera</option>{activeUsers.map(user => <option key={user.id} value={user.id}>{user.name}</option>)}</Select></label><label className="grid gap-1 text-[11px] text-wa-muted">Etiqueta<Select value={form.conditions.tag_id ?? ''} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, tag_id: event.target.value ? Number(event.target.value) : null } }))} className="rounded-lg border border-wa-border bg-white px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark"><option value="">Cualquiera</option>{tags.map(tag => <option key={tag.id} value={tag.id}>{tag.name}</option>)}</Select></label><div className="flex flex-col justify-end gap-2 pb-1"><label className="flex items-center gap-2 text-[11px] text-gray-600 dark:text-gray-300"><Checkbox checked={!!form.conditions.require_open_window} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, require_open_window: event.target.checked } }))} />Ventana de WhatsApp abierta</label><label className="flex items-center gap-2 text-[11px] text-gray-600 dark:text-gray-300"><Checkbox checked={!!form.conditions.business_hours_only} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, business_hours_only: event.target.checked } }))} />Horario laboral 08:00–18:00</label></div><label className="grid gap-1 text-[11px] text-wa-muted md:col-span-3">No repetir para el mismo lead durante (minutos)<input type="number" min={1} max={43200} placeholder="Sin límite" value={form.conditions.cooldown_minutes ?? ''} onChange={event => setForm(current => ({ ...current, conditions: { ...current.conditions, cooldown_minutes: event.target.value ? Number(event.target.value) : null } }))} className="rounded-lg border border-wa-border px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark" /><span className="font-normal text-[10px] text-wa-muted">Evita que se le mande la misma automatización varias veces si escribe seguido. Déjalo vacío para no limitar.</span></label></div></section>

          <section><div className="mb-2 flex items-center justify-between"><h3 className="flex items-center gap-1.5 text-xs font-semibold text-gray-700 dark:text-wa-text-dark"><Activity className="h-3.5 w-3.5" />Acciones en orden</h3><button type="button" disabled={form.actions.length >= 10} onClick={() => setForm(current => ({ ...current, actions: [...current.actions, defaultAction(ActionType.CreateTask)] }))} className="flex items-center gap-1 text-xs font-semibold text-wa-primary-strong disabled:opacity-40 dark:text-wa-primary"><Plus className="h-3.5 w-3.5" />Agregar</button></div><div className="space-y-3">{form.actions.map((action, index) => <div key={index} className="rounded-xl border border-violet-200 bg-violet-50/30 p-3 dark:border-violet-900 dark:bg-violet-950/10"><div className="mb-3 flex items-center gap-2"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-violet-600 text-[10px] font-bold text-white">{index + 1}</span><Select value={action.type} onChange={event => { if (isAutomationActionType(event.target.value)) setAction(index, defaultAction(event.target.value)) }} className="min-w-0 flex-1 rounded-lg border border-violet-200 bg-white px-2.5 py-2 text-xs dark:border-violet-900 dark:bg-wa-head-dark">{Object.entries(ACTION_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Select><button type="button" disabled={form.actions.length === 1} onClick={() => setForm(current => ({ ...current, actions: current.actions.filter((_, itemIndex) => itemIndex !== index) }))} className="rounded-lg p-2 text-red-500 disabled:opacity-30"><Trash2 className="h-4 w-4" /></button></div>
            {action.type === ActionType.CreateTask && <div className="grid gap-2 md:grid-cols-3"><label className="grid gap-1 text-[10px] text-wa-muted dark:text-wa-muted-dark md:col-span-2">Título de la tarea<input required maxLength={160} value={action.title} onChange={event => setAction(index, { ...action, title: event.target.value })} placeholder="Ej. Dar seguimiento a {{nombre}}" className="rounded-lg border border-wa-border px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark" /></label><label className="grid gap-1 text-[10px] text-wa-muted dark:text-wa-muted-dark">Tipo de tarea<Select value={action.task_type} onChange={event => { const value = event.target.value; if (isTaskType(value)) setAction(index, { ...action, task_type: value }) }} className="rounded-lg border border-wa-border bg-white px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark">{TASK_TYPES.map(type => <option key={type.value} value={type.value}>{type.label}</option>)}</Select></label><label className="grid gap-1 text-[10px] text-wa-muted dark:text-wa-muted-dark md:col-span-3">Descripción<input maxLength={1000} value={action.description ?? ''} onChange={event => setAction(index, { ...action, description: event.target.value })} placeholder="Opcional" className="rounded-lg border border-wa-border px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark" /></label><label className="grid gap-1 text-[10px] text-wa-muted dark:text-wa-muted-dark">Vence en minutos<input type="number" min={1} max={43200} value={action.due_minutes} onChange={event => setAction(index, { ...action, due_minutes: Number(event.target.value) })} className="rounded-lg border border-wa-border px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark" /></label><label className="grid gap-1 text-[10px] text-wa-muted dark:text-wa-muted-dark">Recordar antes (min)<input type="number" min={0} value={action.remind_minutes_before} onChange={event => setAction(index, { ...action, remind_minutes_before: Number(event.target.value) })} className="rounded-lg border border-wa-border px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark" /></label><label className="grid gap-1 text-[10px] text-wa-muted dark:text-wa-muted-dark">Responsable<Select value={action.assigned_user_id ?? ''} onChange={event => setAction(index, { ...action, assigned_user_id: event.target.value ? Number(event.target.value) : null })} className="rounded-lg border border-wa-border bg-white px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark"><option value="">Vendedor del lead</option>{activeUsers.map(user => <option key={user.id} value={user.id}>{user.name}</option>)}</Select></label><label className="grid gap-1 text-[10px] text-wa-muted dark:text-wa-muted-dark">Prioridad<Select value={action.priority} onChange={event => { const value = event.target.value; if (isTaskPriority(value)) setAction(index, { ...action, priority: value }) }} className="rounded-lg border border-wa-border bg-white px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark"><option value={TaskPriorityValue.Low}>Baja</option><option value={TaskPriorityValue.Normal}>Normal</option><option value={TaskPriorityValue.High}>Alta</option></Select></label></div>}
            {action.type === ActionType.AssignSeller && <Select required value={action.user_id ?? ''} onChange={event => setAction(index, { ...action, user_id: Number(event.target.value) || null })} className="w-full rounded-lg border border-wa-border bg-white px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark"><option value="">Selecciona vendedor</option>{activeUsers.map(user => <option key={user.id} value={user.id}>{user.name}</option>)}</Select>}
            {(action.type === ActionType.AddTag || action.type === ActionType.RemoveTag) && <Select required value={action.tag_id ?? ''} onChange={event => setAction(index, { ...action, tag_id: Number(event.target.value) || null })} className="w-full rounded-lg border border-wa-border bg-white px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark"><option value="">Selecciona etiqueta</option>{tags.map(tag => <option key={tag.id} value={tag.id}>{tag.name}</option>)}</Select>}
            ...
            {action.type === ActionType.ChangeService && <div><Select value={action.service ?? ''} onChange={event => setAction(index, { ...action, service: event.target.value || null })} className="w-full rounded-lg border border-wa-border px-2.5 py-2 text-xs dark:border-wa-border-dark dark:bg-wa-head-dark"><option value="">Quitar servicio de interés</option>{action.service && !leadServices.some(service => service.name === action.service) && <option value={action.service}>{action.service} (valor actual)</option>}{leadServices.map(service => <option key={service.id} value={service.name}>{service.name}</option>)}</Select><p className="mt-1 text-[10px] text-wa-muted">Selecciona un servicio del catálogo o deja vacío para quitar el actual.</p></div>}
