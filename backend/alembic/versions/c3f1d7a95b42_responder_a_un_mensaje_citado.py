...
La resolucion a la fila local se hace por (chat_id, wa_message_id) al servir
el historial, de ahi el indice: sin el, cada pagina de mensajes hace un
recorrido completo de la tabla por cada cita.
...
def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS idx_wsp_messages_chat_wa_message")
