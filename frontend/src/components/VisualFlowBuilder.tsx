...
function initialFlow(): AutomationFlowDefinition {
  return {
    ...
    nodes: [
      ...
      { id: 'end-1', type: NodeType.End, position: { x: 710, y: 220 }, data: { label: 'Fin', close_conversation: false } },
    ],
    edges: [
  ...
  return {
    ...
    nodes: definition.nodes.map(node => {
      ...
      if (node.type === NodeType.End) {
        return { ...node, data: { ...node.data, close_conversation: node.data.close_conversation === true } }
      }
...
function createFlowNode(type: AutomationFlowNodeType, id: string, x: number, y: number): AutomationFlowNode {
  ...
  switch (type) {
    ...
    case NodeType.End:
      return { id, type, position, data: { label: 'Fin', close_conversation: false } }
    default:
      return assertNever(type)
  }
}
...
export function VisualFlowBuilder({ rule, onClose }: VisualFlowBuilderProps) {
  ...
  const hasUnsavedChanges = savedDefinition == null
    || name !== savedName
    || !areFlowDefinitionsEqual(flow, savedDefinition)
  ...
  useEffect(() => {
    function handleBuilderShortcut(event: KeyboardEvent) {
      ...
      } else if (key === 'd' && selected && selected.type !== NodeType.Trigger) {
        event.preventDefault()
  ...
  return <div className="fixed inset-0 z-[60] flex flex-col bg-wa-field dark:bg-flow-canvas-dark">
    <header className="flex flex-wrap items-center gap-3 border-b border-wa-border bg-white px-4 py-3 dark:border-flow-border-dark dark:bg-flow-panel-dark">
      ...
      {isManualTrigger && (
        <label
          ...
          className="flex items-center gap-1.5 rounded-lg border border-wa-border px-3 py-2 text-xs font-semibold text-gray-600 dark:border-white/10 dark:bg-white/5 dark:text-gray-300"
        >
          <Checkbox
    ...
    <div className="flex min-h-0 flex-1">
      <aside className="w-48 shrink-0 overflow-y-auto border-r border-wa-border bg-white p-3 dark:border-flow-border-dark dark:bg-flow-panel-dark">
        <p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-wa-muted dark:text-wa-muted-dark">Arrastra al lienzo</p>
        {([
        ...
        ] as Array<[AutomationFlowNodeType, string, typeof Activity, string]>).map(([type, label, Icon, description]) => <div key={type} draggable onDragStart={event => event.dataTransfer.setData('application/x-flow-palette', type)} className="group mb-2 cursor-grab rounded-xl border border-wa-border bg-wa-hover p-3 active:cursor-grabbing dark:border-transparent dark:bg-transparent dark:hover:bg-white/5 dark:transition-colors"><div className="flex items-center gap-2 text-xs font-semibold text-gray-800 dark:text-wa-text-dark"><Icon className="h-4 w-4 text-gray-500 transition-colors group-hover:text-wa-primary-strong dark:text-gray-400 dark:group-hover:text-wa-primary" />{label}</div><p className="mt-1 text-[10px] text-wa-muted dark:text-wa-muted-dark">{description}</p></div>)}
      </aside>

      <main className="min-w-0 flex-1 bg-wa-field dark:bg-flow-canvas-dark">
        <FlowCanvas
          flow={flow}
      ...
      {(showEntryConditions || selected) && <aside className="w-80 shrink-0 overflow-y-auto border-l border-wa-border bg-white p-4 dark:border-wa-border-dark dark:bg-wa-panel-dark">
        {showEntryConditions ? <EntryConditionsEditor conditions={flow.conditions} updateConditions={updateEntryConditions} users={activeUsers} tags={tags} /> : selected && <><div className="mb-4 flex items-start gap-2">{nodeIcon(selected.type)}<div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-wa-text dark:text-white">{nodeTitle(selected, automationRules)}</p><p className="text-[10px] text-wa-muted">Propiedades del bloque</p></div>{selected.type !== NodeType.Trigger && <button type="button" onClick={() => duplicateNode(selected)} title="Duplicar nodo (Ctrl/Cmd+D)" aria-label="Duplicar nodo" className="flex shrink-0 items-center gap-1 rounded-lg border border-wa-border px-2 py-1.5 text-[10px] font-semibold text-gray-600 hover:bg-wa-hover dark:border-wa-border-dark dark:text-gray-300 dark:hover:bg-wa-head-dark"><Copy className="h-3.5 w-3.5" />Duplicar</button>}</div>
