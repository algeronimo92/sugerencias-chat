...
export function VisualFlowBuilder({ rule, onClose }: VisualFlowBuilderProps) {
  ...
  const removeNode = useCallback((nodeId: string) => {
    ...
    setFlow(current => {
      ...
      if (node?.type === NodeType.Trigger) {
        setError('El disparador no se puede eliminar; puedes cambiar su tipo.')
        return current
      }
      return {
        ...current,
  ...
  return <div className="fixed inset-0 z-[60] flex flex-col bg-gray-100 dark:bg-gray-950">
    <header className="flex flex-wrap items-center gap-3 border-b border-gray-200 bg-white px-4 py-3 dark:border-gray-800 dark:bg-gray-900">
      ...
      <label title={ruleId == null ? 'Guarda el borrador primero' : 'Freno de seguridad: pasado el tope, el resto se reintenta más tarde'} className="flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold text-gray-600 dark:border-gray-700 dark:text-gray-300">Máx/hora<input type="number" min={1} max={1000} disabled={ruleId == null} placeholder="∞" value={maxPerHour ?? ''} onChange={event => setMaxPerHour(event.target.value ? Number(event.target.value) : null)} onBlur={event => commitMaxPerHour(event.target.value ? Number(event.target.value) : null)} className="w-16 rounded border border-gray-200 bg-white px-1.5 py-1 text-xs disabled:opacity-40 dark:border-gray-700 dark:bg-gray-800" /></label>
      ...
      <button type="button" disabled={ruleId == null} title={ruleId == null ? 'Guarda el borrador primero' : 'Ver versiones publicadas'} onClick={() => setVersionsOpen(true)} className="flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold text-gray-600 disabled:opacity-40 dark:border-gray-700 dark:text-gray-300"><History className="h-4 w-4" />Versiones</button>
    ...
    {(error || notice) && <div className={`px-4 py-2 text-xs ${error ? 'bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300' : 'bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-300'}`}>{error ?? notice}</div>}
    ...
    <div className="flex min-h-0 flex-1">
      ...
      <main className="min-w-0 flex-1 bg-gray-100 dark:bg-gray-950">
        <FlowCanvas
          flow={flow}
    ...
    {versionsOpen && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4" onMouseDown={event => { if (event.target === event.currentTarget) setVersionsOpen(false) }}>
      <div className="max-h-[85vh] w-full max-w-md overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl dark:bg-gray-900">
        <div className="flex items-start justify-between">
          <div>
            <h2 className="flex items-center gap-2 text-sm font-semibold text-gray-900 dark:text-white"><History className="h-4 w-4 text-green-600" />Versiones publicadas</h2>
            <p className="mt-1 text-[11px] text-gray-500">Restaurar carga la versión como borrador. No cambia lo que se está ejecutando hasta que publiques.</p>
          </div>
          <button type="button" onClick={() => setVersionsOpen(false)} className="text-gray-400"><X className="h-5 w-5" /></button>
        </div>
        <div className="mt-4 space-y-2">
          {versionsLoading ? <div className="flex justify-center py-8"><Loader2 className="h-5 w-5 animate-spin text-gray-400" /></div>
            : versions.length === 0 ? <p className="py-8 text-center text-xs text-gray-500">Este flujo todavía no se publicó ninguna vez.</p>
            : versions.map(item => (
              <div key={item.version} className={`flex items-center gap-3 rounded-xl border px-3 py-2.5 ${item.is_current ? 'border-green-300 bg-green-50 dark:border-green-800 dark:bg-green-950/30' : 'border-gray-200 dark:border-gray-700'}`}>
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-semibold text-gray-900 dark:text-white">Versión {item.version}{item.is_current && <span className="ml-2 rounded-full bg-green-600 px-1.5 py-0.5 text-[9px] font-bold text-white">En uso</span>}</p>
                  <p className="text-[10px] text-gray-500">{item.node_count} bloques · {item.edge_count} conexiones · {new Date(item.created_at).toLocaleString('es-PE', { dateStyle: 'short', timeStyle: 'short' })}</p>
                </div>
                <button type="button" disabled={restoreVersion.isPending} onClick={() => void restore(item.version)} className="shrink-0 rounded-lg border border-gray-200 px-2.5 py-1.5 text-[11px] font-semibold text-gray-600 hover:bg-gray-50 disabled:opacity-40 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">Restaurar</button>
              </div>
            ))}
        </div>
      </div>
...
function EntryConditionsEditor({ conditions, updateConditions, users, tags }: EntryConditionsEditorProps) {
  return <div className="space-y-4">
    ...
    <label className="grid gap-1 text-[10px] text-gray-500">No repetir para el mismo lead durante (minutos)
      ...
      <span className="font-normal text-[10px] text-gray-400">Evita que el flujo se repita para el mismo lead si escribe varias veces seguidas. Muy recomendable en flujos disparados por "Mensaje recibido".</span>
    </label>
